...
class PerlinService {
  ...
  constructor() {
    ...
    this.seedZ &= (2 ** 16 - 1);
    seed++;
    // console.log(`Perlin ${seed} with seed ${this.seedX}, ${this.seedY}, ${this.seedZ}`);
  }
...
let perlin: PerlinService[] = range(64).map(() => new PerlinService());;

// const blocksPerChunk = 64;
function getChunk(x: number, y: number, chunksize: number): number[][] {
  const chunk = [];
  for (let i = 0; i < chunksize; i++) {
    const row = [];
    for (let j = 0; j < chunksize; j++) {
      row.push(getValueAtPoint(x * chunksize + i, y * chunksize + j));
    }
    chunk.push(row);
  }
  return chunk;
...
function multiPerlinOctave(x: number, y: number): number {
  const maxOcean = 0.52;
  const minContinent = 0.56;
  const maxOceanLevel = 8;
  const minContinentLevel = 56;
  const maxLevel = 64;

  let continentLayer = (perlin[0]).offset2DOctave(x, y, 8, 1 / 128);
  // if(continentLayer < .48) {
  //   continentLayer *= 10;
  // } else if(continentLayer < 0.52) {
  //   continentLayer = (continentLayer - .48) * 22.4 * (1 / (.52 - .48)) + 4.8;
  // } else {
  //   continentLayer = (continentLayer - .52) * 10 + 27.2;
  // }
  // continentLayer *= 2;

  if(continentLayer < maxOcean) {
    continentLayer *= maxOceanLevel / maxOcean;
  } else if(continentLayer < minContinent) {
    continentLayer = (continentLayer - maxOcean) * (minContinentLevel - maxOceanLevel) / (minContinent - maxOcean) + maxOceanLevel;
  } else {
    continentLayer = (continentLayer - minContinent) * (maxLevel - minContinentLevel) + minContinentLevel;
  }
  ...
  const layer0 = (perlin[1]).offset2DOctave(x, y, 8, 1 / 64) * 32;
  const layer1 = (perlin[2]).offset2DOctave(x, y, 8, 1 / 16) * 32;
  const layer2 = (perlin[3]).offset2DOctave(x, y, 8, 1 / 8) * 8;

  const ravineLayer0 = (perlin[4]).perlin2DOctave(x, y, 8, 1 / 128);
  const ravineLayer1 = (perlin[5]).perlin2DOctave(x, y, 8, 1 / 128);
  const ravineLayer2 = (perlin[6]).perlin2DOctave(x, y, 8, 1 / 256) * 10 + 10;
  // const finalRavineLayer = Math.abs(.5 - ravineLayer0) < 0.02 && ravineLayer1 > 0.5 ? -1 * ravineLayer2 : 0;
  const finalRavineLayer = 0;

  const mountainLayerBase = (perlin[7]).offset2DOctave(x, y, 8, 1 / 64);
  const mountainLayer = mountainLayerBase > 0.55 ? (mountainLayerBase - 0.55) * 140 * (continentLayer / 64) : 0;

  const tempsum = continentLayer + layer0 + layer1 + layer2 + finalRavineLayer + mountainLayer;
  if(tempsum < 64) { 
    if(tempsum < 0) {
      return 0;
    }
...
export function updateSeed(newSeed: number): void {
  seed = newSeed;
  perlin = range(64).map(() => new PerlinService());
}
